...
function sys.wait(ms)
    ...
    -- 调用core的rtos定时器
    if 1 ~= sysLuaStartTimer(timerid, ms) then print('sys.StartTimer error') return end
    -- 挂起调用的任务线程
    local message = {coroutine.yield()}
    if #message ~= 0 then
        sysLuaStopTimer(timerid)
...
function sys.timerStart(fnc, ms, ...)
    ...
    --调用底层接口启动定时器
    if sysLuaStartTimer(msgId, ms) ~= 1 then print('sys.StartTimer error') return end
...
function sys.trigger(param)
    if param < TASK_TIMER_ID_MAX then
        ...
        --如果是循环定时器，继续启动此定时器
        if loop[param] then sysLuaStartTimer(param, loop[param]) end
    end
end
...
--外部触发
function sysTriggerCB(id,t,data)
