...
```lua
transfer(CS, spiID, string.char(0x06))
sys.wait(100)
transfer(CS, spiID, string.char(0x20, 0x00, 0x00, 0x01))
-- 擦除需要消耗一定的时间
sys.wait(1000)
```
...
transfer(CS, spiID, string.char(0x06))
sys.wait(100)
transfer(CS, spiID, string.char(0x02, 0x00, 0x00, 0x01) .. PROJECT)
sys.wait(100)
...
```lua
transfer(CS, spiID, string.char(0x04))
gpio.close(CS_GPIO)
spi.close(spiID)
```
